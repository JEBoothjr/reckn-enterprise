...
# __ - The double underscore is required by Flyway to separate the version from the note

# The second portion, after the double underscore is a note. It must be alpha/numerics and can include spaces.
...
# Validate note format (allow only letters, numbers, spaces, underscores and periods - no double underscores)
if [[ ! "$DESCRIPTION" =~ ^[a-zA-Z0-9\ \._]+$ ]]; then
  echo "Error: Note must be letters, numbers, spaces, underscores and periods - no double underscores"
  exit 1
fi
