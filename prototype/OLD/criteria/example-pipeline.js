const { generateCriteria } = require("./criteria-generator");
const { evaluate } = require("./index");

/**
 * Full Pipeline Example
 *
 * Step 1: JD → Criteria (with weights)
 * Step 2: Resume + Criteria → Evaluation
 */
...
async function main() {
  // === STEP 1: Generate criteria from JD ===

  const jobDescription = `
At ClickUp, we're not just building software. We're architecting the future of work!

ClickUp is seeking a Senior Director of Engineering to lead and inspire multiple engineering teams in building the future of productivity. In this high-impact leadership role, you'll drive the execution, morale, hiring, and technical excellence of 2+ teams, balancing the velocity of feature development with the highest standards of quality and performance.

The ideal candidate is a hands-on technical leader with deep expertise in backend system design or frontend component architecture, operational rigor, and a passion for developing high-performing teams.

The Role:
* Lead, mentor, and manage 2+ engineering teams, ensuring high morale, strong execution, and technical excellence
* Drive the velocity of feature development while maintaining quality, reliability, and performance
* Oversee hiring, onboarding, and professional development for your teams
* Balance operational urgency with long-term technical strategy
...
* Excellent communication, collaboration, and stakeholder management skills
  ...
  console.log("=== STEP 1: Generating Criteria from JD ===\n");

  const criteriaResult = await generateCriteria({
    jobDescription,
    maxCriteria: 7,
  });
  ...
  criteriaResult.criteria.forEach((c) => {
    console.log(`  - ${c.name} (${(c.weight * 100).toFixed(0)}%): ${c.description.slice(0, 60)}...`);
  });
  console.log(`\nReasoning: ${criteriaResult.reasoning}\n`);

  // === STEP 2: Evaluate resume against criteria ===
...
AS, Computer Animation/Multimedia · (1995 - 1997)
  ...
  // Output full JSON for engine consumption
  console.log("=== FULL OUTPUT (for engine) ===\n");
