...
function getScoreObject(rawScore, numFactors) {
    const scale = numFactors;
    const bounds = {
        1: [0, 0.35 * scale],
        2: [0.35 * scale, 3.5 * scale],
        3: [3.5 * scale, 75 * scale],
        4: [75 * scale, 2300 * scale],
        5: [2300 * scale, Infinity]
    };
    ...
    for (let band = 1; band <= 5; band++) {
        ...
        if (rawScore >= min && rawScore <= max) {
            ...
            const logScore = Math.log(rawScore);
...
export function computeDecision(factorsSet) {
    ...
    return {
        engine: VERSION,
        score: getScoreObject(finalScore, factorsSet.length),
        data: results
    };
}